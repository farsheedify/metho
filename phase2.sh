...
run_phase2() {
    ...
    if [[ ! -s "$root_domains_file" ]]; then
        log_error "No root domains file provided for Phase 2"
        return 1
    fi

    local domain_count
    domain_count=$(wc -l < "$root_domains_file")
    log_info "═══ PHASE 2: Root Domains → Subdomains (${domain_count} domains) ═══"
...
process_domain() {
    ...
    subfinder -d "$domain" -all -silent -o subfinder_results.txt 2>/dev/null || true
    ...
    curl -s "https://crt.sh/?q=%25.${domain}&output=json" | \
        ...
        sed 's/\*\.//g' | sort -u > crtsh_results.txt || true
